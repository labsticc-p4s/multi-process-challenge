...
Ici: un paragraphe qui décrit l'exécution, l'activation et la finalisation des taches, la business logic des gateways, etc...
...
Figure \ref{fig:ToolingArchitecture} shows the architecture of these two tools. Left of figure presents process type graphical editor. ProcessTypeEditor is implemented as a VirtualModel declaring two model slots (represented with bold circles). The first model slot references an instance of Acme metamodel, while the second model slot references an instance of diagram, conforms to the ProcessType diagram specification (a diagram metamodel
