...
A UML like representation of a simplified metamodel of \FML is provided in Figure~\ref{fig:mm}. \FML is language designed to define models as virtual models. A virtual model
...
also also the collection of actions that can be performed on them.
These actions are called \emph{behaviors} and can either be called (like usual methods) or triggered by events. The reactive behaviors are mainly useful when a federated model evolution needs to trigger a computation. 
%We do not exploit this possibility in the challenge.
...
\noteSylvain{Rédiger l'explication...}
