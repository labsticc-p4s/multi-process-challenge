...
%
The following listing shows the \FML code of this \textit{virtual model}. {\color{red}Notice that the composition relation of figure~\ref{fig:BPMNSubsetExample} between \texttt{BPMNProcess} and \texttt{BPMNTask} is realized in the FML code by concept containment. Whenever an instance of \texttt{BPMNTask} is created, it must be in a container (instance of \texttt{BPMNProcess}) and the two instances get linked together.}\noteFabien{A valider}
