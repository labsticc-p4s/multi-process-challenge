...
\begin{itemize}
    \item \emph{Conceptualization} (\textit{Conc.}): a concept is created to
      fulfill or meet the requirement (for example for \textbf{P1}:
      \textit{process type} is conceptualized in a concept
    \texttt{ProcessType}).

    \item \emph{Specialization} (\textit{Spec.}): a concept is specialized in
      the sense of object-oriented modeling (for example for \textbf{P2}:
...
business logic).
    ...
    \item \emph{Composition} (\textit{Comp.}): some concepts are in relation,
      ...
      \textbf{P3}: a \textit{process type} has one \textit{initial task type}).
    ...
    \item \emph{Linguistic instantiation} (\textit{L.Inst.}): a requirement is
      ...
      \textbf{S1}: \inst{Requirement analysis} is defined as an instance of
    \texttt{SETaskType}).
    ...
    \item \emph{Ontological instantiation} (\textit{O.Inst.}): fulfillment of a
      ...
      \textbf{S3} is implemented by both a concept in \textit{Acme metamodel}
  ...
  with constraints checking and associated tools (for example \textbf{P17}).
  \end{itemize}
