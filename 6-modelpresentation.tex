...
\texttt{taskTypes} with (0..*) cardinality (\textbf{P1}). A \texttt{TaskType}
...
\texttt{AndJoin}, \texttt{OrSplit} and \texttt{OrJoin} concepts (\textbf{P2}).
...
(cardinality 0..*) (\textbf{P3}). \texttt{TaskType} exposes a creator role as a
reference to an \texttt{Actor} concept (\textbf{P4}), which is at the same
conceptual level.
...
shown in the following \FML code:
...
Enforcing those constraints is finally performed by the definition of this
invariant in the \texttt{Task} concept:
...
relation defined in \texttt{ActorType} (\textbf{P18}). This is completed by
...
(\textbf{S1}). Similarly, \inst{Test case design} is an instance of
...
\inst{Test cases} instance of \texttt{SETaskType} (\textbf{S2}). The latter is
...
\texttt{versionNumber} (an integer). It thus fulfills \textbf{S10}. \inst{Bob
