...
\noteJC{autre remarque qui n'a rien à voir: si on suit les recommandations de l'appel, ce paragraphe devrait se retrouver dans la partie analyse étant donné qu'il établit un choix/une interprétation. Mais c'est délicat à intégrer sans le contexte du modèle de cette section. Ou alors il faudrait une liste, avec des références en avant}
...
\todo[inline]{Augmenter la taille des fontes de la figure \ref{fig:AcmeFullArchitecture} <- a priori non}
\todo[inline]{Ajouter les programming languages <- je les vois apparaitre dans la figure, ou alors c'était un todo pour autre chose ?}
...
\noteFabien{pas de Test case design dans la figure}
