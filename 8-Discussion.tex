...
sous-sections : bof, p-e plutôt des paragraphes ; très court/concis, faudrait
étoffer}
...
\todo[inline]{mandatory discussion aspects ↓}
  ...
  \noteJC{pas clair pour moi ce qui est entendu par « basic modeling constructs »}
...
Our tooling includes mechanisms to verify the models. First, syntactical consistency is realised by cardinality checks and by typing. Second, the constraints the users have written are continuously checked in order to 
...
\todo[inline]{verification de cohérence syntaxique (cardinalités, type des instances) et des contraintes métier (comportements écrits par les humains) à la volée + validation manuelle ; typage ? ; pas d'analyseur statique ; vérif de la structure des contraintes/bonne forme des contraintes par rapport à la structure des modèles (exemple binding pas valide)}
