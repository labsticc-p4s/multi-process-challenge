...
Our modeling approach is based on a language (\FML) which provides the (linguistics) concepts that are used to define  the multi-levels models and meta-models that are needed. \FML allows to define models as first class entities (which we call \emph{virtual models}) and to link them together (by extension). In the \mpc case study, we exploit this possibility to:
\begin{itemize}
